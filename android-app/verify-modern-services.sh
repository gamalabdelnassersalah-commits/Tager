...
grep -q 'canNotifyChannel' "$NOTIFY"
grep -q 'getNotificationChannel' "$NOTIFY"
grep -q 'NotificationManager.IMPORTANCE_NONE' "$NOTIFY"
grep -q 'if (!canNotifyChannel(context, channelId)) return;' "$NOTIFY"
...
grep -q 'buildWebViewHealthCard' "$SETTINGS"
grep -q 'refreshWebViewStatus' "$SETTINGS"
grep -q 'currentWebViewPackage' "$SETTINGS"
grep -q 'openCurrentWebViewSettings' "$SETTINGS"
grep -q 'فتح إعدادات WebView الحالية' "$SETTINGS"
grep -q 'buildHealthSummaryCard' "$SETTINGS"
grep -q 'refreshHealthSummary' "$SETTINGS"
grep -q 'الحالة العامة: سليمة — 4/4 مؤشرات تعمل بشكل طبيعي' "$SETTINGS"
grep -q 'الحالة العامة: تحتاج انتباه' "$SETTINGS"
grep -q 'Health: network=' "$SETTINGS"
grep -q 'canNotifyChannel(this, TagerApplication.CHANNEL_MESSAGES)' "$SETTINGS"
grep -q 'قناة الرسائل متوقفة' "$SETTINGS"
grep -q 'healthRepairButton' "$SETTINGS"
grep -q 'repairFirstHealthIssue' "$SETTINGS"
grep -q 'openNetworkSettings' "$SETTINGS"
grep -q 'Settings.ACTION_WIRELESS_SETTINGS' "$SETTINGS"
grep -q 'openWebViewRecovery' "$SETTINGS"
grep -q 'android.settings.WEBVIEW_SETTINGS' "$SETTINGS"
grep -q 'openFirstNotificationIssue' "$SETTINGS"
grep -q 'مشاركة تقرير التشخيص' "$SETTINGS"
