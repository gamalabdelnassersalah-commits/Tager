...
public class MainActivity extends Activity {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        setIntent(intent);
        if (intent.hasExtra(EXTRA_SAFE_MODE)) safeMode = intent.getBooleanExtra(EXTRA_SAFE_MODE, false);
    ...
    private void rememberGoodUrl(String url) {
        if (url != null && isTagerUrl(url) && !url.equals(preferences.getString(PREF_LAST_GOOD_URL, ""))) {
    ...
    private boolean isTagerUrl(String url) {
        try {
            return url != null && isTagerHost(Uri.parse(url).getHost());
        } catch (Exception ignored) {
            return false;
        }
    }
    ...
    private void configureWebView() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !lowRam && !safeMode) settings.setOffscreenPreRaster(true);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            settings.setSafeBrowsingEnabled(true);
            boolean conserveRenderer = lowRam || safeMode;
            webView.setRendererPriorityPolicy(conserveRenderer ? WebView.RENDERER_PRIORITY_WAIVED : WebView.RENDERER_PRIORITY_BOUND, conserveRenderer);
        }
    ...
    private void scheduleSlowLoadWarning() {
        ...
        slowLoadWarning = () -> {
            if (webView != null && webView.getProgress() < 85) {
                showStatus(isOnline() ? "التحميل أبطأ من المعتاد — اضغط لإعادة المحاولة" : "الاتصال غير مستقر — نحاول استخدام المحتوى المحفوظ");
            }
        };
    ...
    private void restartAfterRendererFailure(String page) {
        ...
        int count = now - last <= RENDER_CRASH_WINDOW_MS ? preferences.getInt(PREF_RENDER_CRASH_COUNT, 0) + 1 : 1;
        preferences.edit().putLong(PREF_RENDER_CRASH_AT, now).putInt(PREF_RENDER_CRASH_COUNT, count).putString(PREF_LAST_PAGE, sanitizePage(page)).apply();
    ...
    private void injectAppPresentation(WebView view) {
        String script = "(function(){document.documentElement.classList.add('tager-native-android');document.body.classList.add('tager-native-android');document.documentElement.style.webkitTextSizeAdjust='100%';if(!window.__tagerNativeLinks){window.__tagerNativeLinks=true;document.addEventListener('click',function(e){var t=e.target;var a=t&&t.closest?t.closest('a[target=\\\"_blank\\\"]'):null;if(a){a.target='_self';}},true);}})();";
    ...
    private Intent createFullResolutionCameraIntent() {
        ...
        try {
            ...
            cameraOutputUri = FileProvider.getUriForFile(this, BuildConfig.APPLICATION_ID + ".fileprovider", image);
    ...
    private void retryCurrentPage() {
        ...
        if (current == null || current.startsWith("about:")) {
            ...
            showLoading(true);
            webView.loadUrl(isTagerUrl(lastGood) ? lastGood : getPageUrl(preferences.getString(PREF_LAST_PAGE, "home")));
        } else webView.reload();
    }
    ...
    public void onTrimMemory(int level) {
        ...
        if (webView != null && webView.getUrl() != null) rememberGoodUrl(webView.getUrl());
    ...
    public void onBackPressed() {
        ...
        long now = System.currentTimeMillis();
        if (now - lastBackPressedAt < 1800L) moveTaskToBack(true);
        else {
